...
import BtnPulse from "../../core/BtnPulse";
import { reducer, reducerInit, ACTION } from "./reducer";
...
const StickyGridAdvanced = ({
  ...
  columnsSetsEnableShared,
  columnsSetsOnSharedUpsert,
  columnsSetsOnSharedDelete,
  columnsSetsGetItemsShared,
  columnsSetsEnableLevels,
  columnsSetsOnLevelsUpdate,
  columnsSetsGetItemsLevels,
  columns,
  columnsDememoized = [],
  columnsSortMax,
  ...
  const resetScrollbarY = React.useMemo(() => {
    ...
    const canReset =
      resetScrollbar === undefined || refResetTime.current !== resetScrollbar;
    if (!rootLoading && canReset && refIdsJoined.current !== newIdsJoined) {
      // quando finisce il loading aggiorno refResetTime
      // in questo modo resetto la scrollbar solo dopo il loading
      refIdsJoined.current = newIdsJoined;
      refResetTime.current = resetScrollbar || new Date().getTime();
    }
    return refResetTime.current;
  }, [resetScrollbar, rootLoading, rows]);
  ...
  const slcSet: IMngSet = React.useMemo(() => {
    return columnsSets.find((s) => s.id === columnsSetsSelectedId);
  }, [columnsSets, columnsSetsSelectedId]);
  ...
  }, [columns, defaultColumnSizes, slcSet]);
  ...
  const sizes = React.useMemo(() => {
    return activeColumns.map((col, index) => {
      if (resizeColumnIndex === index) return resizeColumnWidth;
      return col.width;
    });
  }, [activeColumns, resizeColumnIndex, resizeColumnWidth]);
  ...
  const onRemove = React.useCallback(
    (colId: string) => {
      dispatch({
        ...
        colId,
        mngSets: columnsSets,
        mngSetsSlcId: columnsSetsSelectedId,
      });
    },
    [columnsSets, columnsSetsSelectedId],
  );
  ...
  const onResizing = React.useCallback(
    (columnWidth: number, columnIndex: number) => {
      dispatch({
        type: ACTION.GRID_ON_RESIZING,
        columnWidth,
        columnIndex,
      });
    },
  ...
  const cbHeaderClick = React.useCallback(
    (sort: ISort[]) => {
      if (sort.length > columnsSortMax) {
        dispatch({ type: ACTION.MODAL_SORTS_ERROR });
      } else {
        dispatch({
          type: ACTION.GRID_ON_SORT,
          sort,
          mngSets: columnsSets,
          mngSetsSlcId: columnsSetsSelectedId,
        });
    ...
    [columnsSets, columnsSetsSelectedId, columnsSortMax],
  );
      ...
      rows.map((item) => {
        const contentData: ICellContentData[] = activeColumns.map((column) => {
          const d: ICellContentData = {
            ...
            dememoize: new Set(columnsDememoized).has(column.id),
            loading: rootLoading,
            column,
  ...
  const topData: IStickTableRow[] = React.useMemo((): IStickTableRow[] => {
    return activeColumns.map(({ id, label, sortable }) => {
      const contentData: ICellHeaderData = {
        value: id + "_" + (!!slcSet ? slcSet.version : 0), // change "value" -> fire rerender topData
        id,
        label,
        sortable,
  ...
  const getManagerChip = React.useCallback(
    (chip: IMngChipValue, _: IMngSetPayload, shared: boolean): IChip => {
      const c: IChip = {
        id: chip.id,
        label: chip.label,
    ...
    (async () => {
      if (draftSetInit === Initialize.START) {
        try {
          ...
          const { id, payload, shareType } = draftSetData;
          const oldSet = columnsSets.find((s) => s.id === id);
          ...
          const newSet = await columnsSetsOnUpdateSet(
            columnsSetsContext,
            setToUpdate,
          );
          const newSets: IMngSet[] = columnsSets.map((s) => {
            if (s.id !== id) return s;
            return newSet;
          });
          ...
          columnsSetsOnChanged(columnsSetsContext, id, newSets);
        } catch {
  ...
  return (
    ...
    >
      ...
      <WindowMngSets
        ...
        loading={inLoading(draftSetInit)}
        ...
        getUser={getUser}
        onError={onError}
        enableShared={columnsSetsEnableShared}
        onSharedUpsert={columnsSetsOnSharedUpsert}
        onSharedDelete={columnsSetsOnSharedDelete}
        getItemsShared={columnsSetsGetItemsShared}
        enableLevels={columnsSetsEnableLevels}
        onLevelsUpdate={columnsSetsOnLevelsUpdate}
        getItemsLevels={columnsSetsGetItemsLevels}
      />
      <Modal
